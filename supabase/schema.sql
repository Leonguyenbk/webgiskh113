...
-- =========================================================
-- ĐÃ GỠ BỎ 2026-09: "Thống kê nhập biểu" (bieu_thong_ke_theo_xa) — trang
-- và toàn bộ endpoint backend đã xóa vì tốn tài nguyên Postgres (2 hàm
-- refresh quét thua_dat/du_lieu_gcn ~3,86 triệu / ~448 nghìn dòng, chạy
-- 2 lần/ngày). Nếu trước đó đã bật pg_cron cho 2 job
-- 'refresh-bieu-thong-ke-1'/'refresh-bieu-thong-ke-2', chạy trong Supabase
-- SQL Editor để hủy lịch trước khi drop hàm:
--   select cron.unschedule('refresh-bieu-thong-ke-1');
--   select cron.unschedule('refresh-bieu-thong-ke-2');
-- Nếu dùng Render Cron Job gọi POST /api/bieu-thong-ke/refresh thì xóa
-- job đó trên Render (endpoint đã không còn tồn tại ở backend).
-- =========================================================

drop function if exists public.refresh_bieu_thong_ke_theo_xa_cache();
drop function if exists public.refresh_bieu_thong_ke_theo_xa_cache_phan2();
drop function if exists public.refresh_bieu_thong_ke_theo_xa_cache_phan1();
drop function if exists public.bieu_thong_ke_theo_xa();
drop table if exists public.bieu_thong_ke_theo_xa_cache;
